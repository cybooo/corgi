        ...
        try {
            ...
            if (purge <= 1) {
                ...
                message.delete().queue(useless -> message.getTextChannel().getHistory().retrievePast(purge).queue(msgsRaw -> {
                    ...
                    message.getTextChannel().sendMessageEmbeds(MessageUtils.getEmbed(Constants.GRAY).setDescription("Deleting messages..").build()).queue(msg -> {
                        if (msgs.size() > 0) {
                            if (args.length >= 2) {
                                ...
                                if (message.getMentionedUsers().size() == 0) {
                                    ...
                                    if (keywordPurge.size() >= 1)
                                        msg.editMessageEmbeds(MessageUtils.getEmbed(Constants.GREEN).setDescription(EmoteList.GREEN_OK + " | Deleted " + keywordPurge.size() + " messages!").build()).queue();
                                    else
                                        msg.editMessageEmbeds(MessageUtils.getEmbed(Constants.RED).setDescription(EmoteList.RED_DENY + " | Could not find messages to delete!").build()).queue();
                                } else {
                                    ...
                                    if (mentions.stream().anyMatch(mentions::contains) && m.find()) {
                                        msg.editMessageEmbeds(MessageUtils.getEmbed(Constants.RED).setDescription(EmoteList.RED_DENY + " | You can't delete mentions and selected text at once!").build()).queue();
                                    } else {
                                        ...
                                        if (mentionPurge.size() >= 1)
                                            msg.editMessageEmbeds(MessageUtils.getEmbed(Constants.GREEN).setDescription(EmoteList.GREEN_OK + " | Succesfully deleted " + mentionPurge.size() + " message(s) from: `" + StringUtils.join(message.getMentionedUsers().stream().map(user -> user.getName() + "#" + user.getDiscriminator()).collect(Collectors.toList()), ", ") + "`").build()).queue();
                                        else
                                            msg.editMessageEmbeds(MessageUtils.getEmbed(Constants.RED).setDescription(EmoteList.RED_DENY + " | Could not find any message from specified users!").build()).queue();
                                    }
                                }
                            } else {
                                if (msgs.size() == 1)
                                    msgs.get(0).delete().queue(delet -> msg.editMessageEmbeds(MessageUtils.getEmbed(Constants.GREEN).setDescription(EmoteList.GREEN_OK + " | Succesfully deleted " + msgs.size() + " messages.").build()).queue());
                                else
                                    message.getTextChannel().deleteMessages(msgs).queue(delet -> msg.editMessageEmbeds(MessageUtils.getEmbed(Constants.GREEN).setDescription(EmoteList.GREEN_OK + " | Succesfully deleted " + msgs.size() + " messages.").build()).queue());
                            }
                        } else
                            msg.editMessageEmbeds(MessageUtils.getEmbed(Constants.RED).setDescription(EmoteList.RED_DENY + " | You can't delete messages older than 14 days!").build()).queue();
                    });
                }));
            }
